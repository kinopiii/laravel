...
<body>
    ...
    <hr color="black">
    <br>
    {{Form::open(['method'=>'POST','route' => ['reviews_delete.post',$items->review_id]])}}
    <span class="bold">商品評価</span> 
    <span class="value value2">{{ $items->evaluation }}</span> 
    <input type="hidden" name="evaluation" value="{{ $items->evaluation }}">
    <br><br>
    <span class="bold">商品コメント</span>
    <span class="value">{{ $items->comment }}</span>
    <input type="hidden" name="comment" value="{{ $items->comment }}">
    <br><br><br>
    {{Form::submit('削除する', ['class'=>'blue-button'])}}<br><br>
